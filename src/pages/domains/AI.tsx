...
const SERVICE_SLUG = 'ai' as ServiceType;
...
const AIDomainPage: React.FC = () => {
  ...
  const title = page?.title || 'Artificial Intelligence';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Exploring the cutting edge of AI, from machine learning models to practical applications and ethical considerations.';
  const canonicalUrl = `https://www.handywriterz.com/d/ai`;
      ...
      <section className="bg-gradient-to-b from-rose-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-rose-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Your hub for the latest in Artificial Intelligence, from deep learning breakthroughs to real-world applications.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['language-models', 'computer-vision', 'generative-ai']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated AI research and clinical notes.'}</p>
                  <Link to={`#cat-${c.slug}`} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-rose-600">Explore<ArrowRight className="h-4 w-4" /></Link>
                </div>
              ))}
            </div>
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['language-models', 'computer-vision', 'generative-ai']).map((tag) => (
