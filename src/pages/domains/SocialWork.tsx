...
const SERVICE_SLUG = 'social-work' as ServiceType;
...
const SocialWorkDomainPage: React.FC = () => {
  ...
  const title = page?.title || 'Social Work';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Professional resources for social workers, including case studies, intervention strategies, and policy updates.';
  const canonicalUrl = `https://www.handywriterz.com/d/social-work`;
      ...
      <section className="bg-gradient-to-b from-amber-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-amber-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Empowering social workers with tools, research, and support to foster community well-being and advocate for social justice.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['community-practice', 'clinical-social-work', 'policy-advocacy']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated social work research and clinical notes.'}</p>
                  <Link to={`#cat-${c.slug}`} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-amber-600">Explore<ArrowRight className="h-4 w-4" /></Link>
                </div>
              ))}
            </div>
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['community-practice', 'clinical-social-work', 'policy-advocacy']).map((tag) => (
...
export default SocialWorkDomainPage;
