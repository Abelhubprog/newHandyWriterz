...
export const normalizeSections = (
  ...
  return sections.map((section: any) => {
    ...
    if (!section.media) return base as EditableSection;
    const typeCandidate = String(section.media.type || 'image');
    const safeType = SUPPORTED_MEDIA_TYPES.includes(typeCandidate as any)
      ? (typeCandidate as EditableSection['media']['type'])
      : 'image';
    return {
      ...base,
      media: {
