...
const ContentList: React.FC = () => {
  ...
  const { data: posts = [], isLoading: isInitialLoading, error: postsError, refetch: refetchPosts } = useQuery<Post[]>({
  ...
  // Fetch popular tags
  const { data: popularTags = [], isLoading: tagsLoading, error: tagsError } = useQuery<string[]>({
    queryKey: ['tags', service],
    queryFn: () => contentManagementService.getPopularTags(service || ''),
    staleTime: 5 * 60 * 1000
  });

  // Aggregated posts for local infinite-scroll append
  const [allPosts, setAllPosts] = useState<Post[]>([]);
  useEffect(() => {
    setAllPosts(posts);
  }, [posts, service, activeCategory, searchQuery]);

  // Filter posts client-side for search (service already handles category)
  const filteredPosts = React.useMemo(() => {
    const source = allPosts.length ? allPosts : posts;
