...
const SERVICE_SLUG = 'crypto' as ServiceType;
...
const CryptoServicePage: React.FC = () => {
  ...
  const title = page?.title || 'Crypto Insights';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Research, market analysis, and Web3 engineering notes for crypto practitioners.';
  const canonicalUrl = `https://www.handywriterz.com/services/${SERVICE_SLUG}`;
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Market structure, protocol design, and on-chain data stories for crypto professionals.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['on-chain', 'defi', 'research']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated crypto research and engineering notes.'}</p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['on-chain', 'defi', 'research']).map((tag) => (
...
export default CryptoServicePage;
