...
const SERVICE_SLUG = 'mental-health-nursing' as ServiceType;
...
const MentalHealthDomainPage: React.FC = () => {
  ...
  const title = page?.title || 'Mental Health Nursing';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Evidence-based practices, therapeutic communication, and patient advocacy resources for mental health nurses.';
  const canonicalUrl = `https://www.handywriterz.com/d/mental-health`;
      ...
      <section className="bg-gradient-to-b from-violet-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-violet-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Supporting mental wellness through compassionate care, evidence-based interventions, and patient advocacy.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['psychotherapy', 'pharmacology', 'crisis-intervention']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated mental health nursing research and clinical notes.'}</p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['psychotherapy', 'pharmacology', 'crisis-intervention']).map((tag) => (
...
export default MentalHealthDomainPage;
