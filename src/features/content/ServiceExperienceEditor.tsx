...
// Use consolidated editor submodules under features namespace
import HeroForm from '@/features/content/service-experience/HeroForm';
import HighlightsForm from '@/features/content/service-experience/HighlightsForm';
import SectionsForm from '@/features/content/service-experience/SectionEditor';
import FaqPricingForm from '@/features/content/service-experience/FaqPricingForm';
import {
	...
	type EditableTier,
} from '@/features/content/service-experience/types';
import { normalizeSections, normalizeTiers } from '@/features/content/service-experience/utils';
...
const ServiceExperienceEditor: React.FC = () => {
	...
	useEffect(() => {
		if (pageData) {
			setPageDraft({
				...
				faq: pageData.faq || [],
				pricing: {
					tiers: normalizeTiers((pageData.pricing?.tiers as any[] | undefined)?.map(t => ({ ...t, isPopular: t?.isPopular ?? false })) as EditableTier[]),
				},
			});
		}
	}, [pageData]);
	...
	const updatePage = (changes: Partial<ServicePageRecord>) => {
		setPageDraft((prev) => {
			if (!prev) return prev;
			const next: ServicePageDraft = {
				...(prev as any),
				...changes,
				pricing: {
					tiers: normalizeTiers((changes as any)?.pricing?.tiers ?? prev.pricing?.tiers),
				},
			};
			return next;
		});
		...
		onSuccess: (savedPage) => {
			if (savedPage) {
				setPageDraft({
					...
					pricing: { tiers: normalizeTiers((savedPage.pricing?.tiers as any[] | undefined)?.map(t => ({ ...t, isPopular: t?.isPopular ?? false })) as EditableTier[]) },
				});
	...
	return (
		<div className="space-y-8 px-4 py-6 sm:px-6 lg:px-8">
			...
			{activeTab === 'highlights' && (
							<HighlightsForm
								highlights={highlightCards as any}
								maxHighlights={HIGHLIGHT_LIMIT}
								onAdd={handleAddHighlight}
								onChange={handleHighlightChange as any}
							...
							<div className="space-y-4">
								{sections.map((section) => (
									<SectionsForm
										key={section.id}
										section={section}
										onChange={(field, value) => handleSectionChange(section.id, field as any, value)}
										onMediaUpload={(file) => {
											const url = URL.createObjectURL(file);
											handleMediaChange(section.id, 'url', url);
										}}
									/>
								))}
								<button type="button" className="text-sm font-semibold text-sky-600" onClick={handleAddSection}>Add section</button>
							</div>
			)}
