...
const Categories: React.FC = () => {
  ...
  const fetchServices = async () => {
    try {
      const svcs = await databaseService.getServices();
  ...
  const handleCreateCategory = async () => {
    try {
      ...
      await databaseService.upsert('categories', {
        id: `cat-${Date.now()}`,
  ...
  const handleUpdateCategory = async (id: string, updates: Partial<Category>) => {
    try {
      await databaseService.upsert('categories', { id, ...updates, updated_at: new Date().toISOString() });
