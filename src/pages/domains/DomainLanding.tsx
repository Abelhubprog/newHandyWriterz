...
import { Search, Filter, User, Heart, MessageCircle, Bookmark, Share2, Loader2, Tag as TagIcon, ArrowRight } from 'lucide-react';
...
import type { ServiceType, Category } from '@/types/content';
...
const DOMAIN_TO_SERVICE: Record<string, ServiceType> = {
  'adult-health': 'adult-health-nursing',
  'mental-health': 'mental-health-nursing',
  'child-nursing': 'child-nursing',
  'social-work': 'social-work',
  // Map technology domain to a concrete service type for now
  'technology': 'crypto',
};
...
const DomainLanding: React.FC = () => {
  ...
  const serviceSlug = DOMAIN_TO_SERVICE[domain] || 'adult-health-nursing';
  ...
  const {
    ...
    serviceType: serviceSlug as ServiceType,
    status: 'published',
    limit: 9,
  ...
  const title = page?.title || domain?.replace(/-/g, ' ').replace(/\b\w/g, (m) => m.toUpperCase()) || 'Domain';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || `${title} resources and services.`;
  const canonicalUrl = `https://www.handywriterz.com/d/${domain}`;
  ...
  const { data: featured = [] } = useQuery({
    queryKey: ['featured-posts-inline', serviceSlug],
    queryFn: () => ServiceContentService.getFeaturedPosts(serviceSlug, 3),
    staleTime: 5 * 60 * 1000,
  });

  const { data: trendingData } = useQuery({
    queryKey: ['service-trending', serviceSlug],
    queryFn: async () => {
      const { posts: all } = await ServiceContentService.getServicePosts(serviceSlug, { status: 'published', limit: 30 });
      ...
      <section className="bg-gradient-to-b from-slate-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['evidence-based', 'quality', 'safety']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated insights and tools.'}</p>
                  <Link to={`#cat-${c.slug}`} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-sky-600">Explore<ArrowRight className="h-4 w-4" /></Link>
                </div>
              ))}
            </div>
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['evidence-based', 'quality', 'safety']).map((tag) => (
