...
const SERVICE_SLUG = 'child-nursing' as ServiceType;
...
const ChildNursingDomainPage: React.FC = () => {
  ...
  const title = page?.title || 'Child Health Nursing';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Specialized resources for pediatric nursing, from neonatal care to adolescent health, focusing on family-centered approaches.';
  const canonicalUrl = `https://www.handywriterz.com/d/child-nursing`;
      ...
      <section className="bg-gradient-to-b from-sky-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-sky-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Dedicated to the health and well-being of children, providing expert guidance on pediatric care, developmental milestones, and family support.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['neonatal', 'pediatric-oncology', 'developmental-care']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated child health nursing research and clinical notes.'}</p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['neonatal', 'pediatric-oncology', 'developmental-care']).map((tag) => (
...
export default ChildNursingDomainPage;
