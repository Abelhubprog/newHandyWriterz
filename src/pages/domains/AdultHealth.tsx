...
const SERVICE_SLUG = 'adult-health-nursing' as ServiceType;
...
const AdultHealthDomainPage: React.FC = () => {
  ...
  const title = page?.title || 'Adult Health Nursing';
  const seoTitle = page?.seo?.title || `${title} | HandyWriterz`;
  const seoDesc = page?.seo?.description || page?.summary || 'Comprehensive resources for adult health nursing, including clinical guidelines, patient care strategies, and professional development.';
  const canonicalUrl = `https://www.handywriterz.com/d/adult-health`;
      ...
      <section className="bg-gradient-to-b from-emerald-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-emerald-500">Domain</div>
              <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">{title}</h1>
              <p className="mt-3 text-lg text-slate-700">{page?.summary || cat?.heroSummary || 'Promoting wellness and managing complex health conditions in adult populations through expert nursing care.'}</p>
        ...
        <section id="articles" className="py-8">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {(tags.length ? tags : ['chronic-disease', 'gerontology', 'palliative-care']).slice(0, 12).map((tag) => (
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(categories as Category[]).map((c) => (
                <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-slate-600">{c.description || 'Curated adult health nursing research and clinical notes.'}</p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(tags.length ? tags : ['chronic-disease', 'gerontology', 'palliative-care']).map((tag) => (
...
export default AdultHealthDomainPage;
