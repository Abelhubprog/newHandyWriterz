...
const ContentEditor: React.FC = () => {
  ...
  const { data: existingPost, isLoading: isLoadingPost } = useQuery({
    queryKey: ['post', id],
    // Fallback: fetch posts by service and filter by id since getBlogPost is not implemented
    queryFn: async () => {
      if (!id) return undefined as any;
      const posts = await contentManagementService.getPostsByService('ai', 1, 0);
  ...
  const saveMutation = useMutation({
    mutationFn: async (postData: Post) => {
      ...
      if (isEditing) {
        return contentManagementService.updatePost(id!, postData as any);
      } else {
        // Not implemented in service; emulate create by erroring gracefully
        throw new Error('Create post is not available in this environment');
      }
    },
    onSuccess: (savedId) => {
